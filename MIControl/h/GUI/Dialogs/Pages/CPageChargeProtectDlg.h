...
class CPageChargeProtectDlg : public CPageDlg
{
    CToolbar		m_tb;
    uint8_t			m_uCurrentMode = 0;
    ...
    CBitmap&		AddHeart(CBitmap& bm);
    void			OnServiceChange(bool bState);
    bool			GetLineColor(COLORREF& clr, UINT& iconId) const override;

public:
    CPageChargeProtectDlg(HINSTANCE hInstance, SendCommand_t, HOST_DATA& status);
};
