...
{	
	...
	if ( sz.cx > 0 && sz.cy > 0 )
	{
		m_pvBits.resize(static_cast<size_t>(sz.cx) * sz.cy);
		m_sz = sz;
		return TRUE;
	}
...
BOOL CIconBits::ImportIcon(HBITMAP hBitmap)
{
	...
	if ( GetObject( hBitmap, sizeof(bmp), &bmp ) )
	{
		...
		m_pvBits.resize( static_cast<size_t>(bmp.bmWidth) * bmp.bmHeight );
...
BOOL CIconBits::GetMaskBits(HBITMAP hBitmap, std::vector<BYTE> & pBits)
{
	...
	if ( GetObject(hBitmap, sizeof(BITMAP), bmp) )	
	{
		...
		pBits.resize(static_cast<size_t>(bmp->bmWidth) * bmp->bmHeight * sizeof(RGBQUAD));
...
SIZE CIconBits::Size() const
{
	return m_sz;
}

LONG CIconBits::Width() const
{
	return m_sz.cx;
}

LONG CIconBits::Height() const
{
	return m_sz.cy;
}
